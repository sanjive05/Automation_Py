...
# Read the Excel file with correct header row (row index 1)import pandas as pd
#
# # Read Excel, skipping the first row. The next row becomes header.
df = pd.read_excel("D:\\Automation_Py\\data\\men_data1.xlsx",header=1,dtype={'PHONE NUMBER': str})
# Check column names
...
for index, row in df.iterrows():
    ...
    # Skip if phone number is empty or NaN
   ...
   #     continue
    ...
    if (len(phone_number) < 10):
        continue
    print(f"Sending to {phone_number}..." ,len(phone_number))
